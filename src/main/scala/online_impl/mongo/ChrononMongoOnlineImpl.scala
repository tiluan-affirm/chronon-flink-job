...
class ChrononMongoOnlineImpl(userConf: Map[String, String]) extends Api(userConf) {
  ...
  @transient lazy val mongoClient =
    MongoClient(s"mongodb://${userConf("user")}:${userConf("password")}@${userConf("host")}:${userConf("port")}")
  ...
  @transient private lazy val loggingClient =
    mongoClient.getDatabase(Constants.mongoDatabase).getCollection(Constants.mongoLoggingCollection)
